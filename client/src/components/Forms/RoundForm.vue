<template>
  <v-form ref="form" style="min-width: 500px" v-model="valid">
    <v-text-field
      ...
      required
      :disabled="!!round && !$store.getters.hasPermission('roundsProps.name')"
    ></v-text-field>
    <DatetimePicker
      ...
      :disabled="!!round && !$store.getters.hasPermission('roundsProps.dates')"
    />
    <DatetimePicker
      :date="draft.endDate"
      label="End date"
    ...
    <v-text-field
      ...
      :disabled="!!round && !$store.getters.hasPermission('roundsProps.bestOf')"
    >
    </v-text-field>
    <v-text-field
      ...
      :disabled="!!round && !$store.getters.hasPermission('roundsProps.prepTime')"
    ></v-text-field>
    <v-row>
      <v-spacer></v-spacer>
      <v-btn color="accent black--text" class="mt-8" text @click="$emit('submit', draft)" :disabled="!valid"> Save </v-btn>
      <v-btn color="accent black--text" class="mt-8" text @click="$emit('cancel')"> Cancel </v-btn>
    </v-row>
  </v-form>
</template>
