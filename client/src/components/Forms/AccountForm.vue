<template>
  <v-form ref="form" style="min-width: 500px">
    ...
    <div v-if="account">
      <v-select :items="presets" background-color="transparent" color="accent" flat prepend-icon="mdi-arrow-down-box" label="Add a preset">
      </v-select>
      <v-textarea color="accent" label="Notes" :value="account.notes"></v-textarea>
    </div>
    <v-row>
      <v-spacer></v-spacer>
