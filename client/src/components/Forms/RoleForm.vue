  ...
  <v-form ref="form">
    <v-card-text>
      <v-row align="center" no-gutters>
        ...
        <v-text-field v-model="draft.name" color="accent" required outlined dense></v-text-field>
      </v-row>
      <v-row align="center" no-gutters>
        <p class="title accent--text flex-grow-1">Role color</p>
        <v-switch v-model="draft.color" label="Transparent" color="accent" true-value="transparent" false-value="#FF0000"></v-switch>
        <v-menu offset-y :close-on-content-click="false" v-if="draft.color !== 'transparent'">
          ...
          <v-color-picker v-model="draft.color" class="mx-auto"></v-color-picker>
        </v-menu>
      </v-row>
      <v-row align="center" no-gutters>
      ...
      <v-row align="center" no-gutters>
        ...
        <v-switch v-model="draft.editable" :label="draft.editable ? 'Yes' : 'No'" color="accent"></v-switch>
      </v-row>
      <p class="title accent--text">Permissions</p>
      <PermissionsList :rolePermissions.sync="draft.permissions" />
    </v-card-text>
